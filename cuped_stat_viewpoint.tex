...
are usually not satisfied in practice, i.e., the conditional ex-
pectation of the outcome metric is linear in the treatment
...
So I diceded to expose the CUPED method using old boring regression language. 
...
Step one. Estimate the regression 
\[
...
Calculate semiresidual $r_i = y_i - \hat \theta x_i$.
...
Step two. Estimate regression 
\[
    \hat r_i = \hat \beta_1 + \hat \beta_2 w_i
\]
 using OLS.

Use classical standard errors to build confidence interval for $\beta_2$.


Why this two-step procedure is better than just plain old multiple regression
\[
    \hat y_i = \hat \gamma_1 + \hat\gamma_2 w_i + \hat \theta x_i
\]
